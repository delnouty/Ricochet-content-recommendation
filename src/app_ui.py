"""Interface Streamlit partagée par les solutions **locale** et **Hugging Face**.

Source unique de vérité : `src/app_ui.py`. Les copies déployées (`local/ui.py`,
`spaces/ui.py`) sont générées par `scripts/sync_recommender.py` — ne pas les éditer.

Chaque hébergement fournit son contexte puis appelle `run()` :

    run(models_dir, clients_db, banniere=None)

  - `models_dir` : dossier d'artefacts — un dossier du dépôt en local, le dossier
    téléchargé depuis le HF Hub sur un Space ;
  - `clients_db` : base SQLite des clients inscrits. Sur un Space elle est
    **éphémère** (perdue au redémarrage) et partagée entre visiteurs, d'où la
    bannière ;
  - `banniere` : avertissement affiché en haut de page, propre à l'hébergement.

Le contexte est gardé dans `_CTX` plutôt que passé à chaque fonction : les vues
appellent `get_recommender()`, `get_store()`, `get_ratings()` sans savoir où elles
tournent. Aucune dépendance cloud : `streamlit`, `numpy`, bibliothèque standard.
"""
