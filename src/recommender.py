"""Système de recommandation d'articles — cœur métier.

Ce module est volontairement **léger** : à l'inférence il ne dépend que de
`numpy` (+ `pickle` de la stdlib). Toute la partie coûteuse (PCA, entraînement
du filtrage collaboratif) est faite hors-ligne par `prepare_model.py` et
...
« architecture cible » pour l'ajout de nouveaux utilisateurs / articles.
"""
